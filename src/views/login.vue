  ...
  methods: {
    ...
    shell: (e) => {
      for (var i = 0; i < this.allButtons.length; i++)
        this.allButtons[i].addEventListener('click', this.getButtons)
      for (var i = 0; i < this.switchBtn.length; i++)
        this.switchBtn[i].addEventListener('click', this.changeForm)
    },
    addListener() {
      window.addEventListener('load', this.shell)
    }
  }
  // 点击切换
  // window.addEventListener("load", shell)
